...
type exp = 
    ...
    | Estring of string		
    | Dict of edic			(*Espressione dizionario				  *)
...
and dictval = Vuoto | Val of (ide * evT) * dictval
...
let rec eval (e : exp) (r : evT env) : evT = match e with
  ...
  |Dict(a) -> let rec scan a1 = match a1 with
                |Empty -> Vuoto
                |Elem((b1, b2), xs) -> Val((b1, eval b2 r), scan xs)
      in DictVal(scan a)
  ...
  |AddInDict(d, id, v) ->   (*Fa uso di una funzione di appoggio per scandire il dizionario (Insert),
      ...
      (let rec insert l = match l with
          ...
          |DictVal(a) -> (match a with

                           |Vuoto -> Val((id, eval v r),Vuoto)
                           |Val(x,xs) -> (let rec reflect x1 = match x1 with
                                            |(a,b) -> (*Se la chiave corrisponde a quella da inserire -> exception*)
                                                if a = id then failwith("Chiave gia presente")
                                                else let r = DictVal(xs) in 
                                                    Val((a, b), insert r)
                                          in reflect x))
          |_ -> failwith("type error")
       in DictVal(insert (d)))
  ...
  |RmDict(d, id) ->   (*Utilizza una funzione interna per scandire il dizionario
      ...
      (let rec remove dicty = 
         match dicty with
           |DictVal(k) -> (match k with
                            |Vuoto -> failwith("Chiave non presente")
                            |Val((a,b),xs) -> if a = id then
                                  (let rec scan di = match di with
                                      |Vuoto -> Vuoto
                                      |Val((c1,c2),ys) -> Val((c1, c2), scan ys)
                                   in scan xs)
                                else let newdict = DictVal(xs) in Val((a,b),remove newdict))
           |_ -> failwith("Type error")
       in DictVal(remove (d)))
  ...
  |ApplyOver(f,d) ->  (*Utilizza una prima funzione interna che scandisce il dizionario
      ...
      (let rec scan l = match l with
          |DictVal(Vuoto) -> Vuoto
          |DictVal(Val(x,xs)) -> let rec apply x1 = match x1 with
                                   |(a,b)-> (a, eval (FunCall(f,toexp(b))) r)
              in 
              let newdictionary = DictVal(xs)
              in Val((apply x),(scan newdictionary))
          |_ -> failwith("Type error")
       in DictVal(scan (d)))
  ...
  |DictAccess(a, i) -> (*accede all'elemento con chiave i in a.
      ...
      let rec valuta b = 
        match b with
          |DictVal(Vuoto) -> failwith("Elemento non presente")
          |DictVal(Val((a,c), Vuoto)) -> if a = i then ElemDict(c)
              else failwith("Elemento non presente")
          |DictVal(Val((a,c), Val(s,t))) -> if a = i then ElemDict(c)
              else (let newdict = DictVal(Val(s,t)) 
                    in valuta newdict)
          |_ -> failwith("Type error")
      in valuta (a);;
...
let rec rt_eval (e : exp) (r : evT env) : evT = match e with
  ...
  |Dict(a) -> let rec scan a1 = match a1 with
                |Empty -> Vuoto
                |Elem((b1, b2), xs) -> Val((b1, rt_eval b2 r), scan xs)
      in DictVal(scan a)
  ...
  |AddInDict(d, id, v) ->   (*Fa uso di una funzione di appoggio per scandire il dizionario (Insert),
      ...
      (let rec insert l = match l with
          ...
          |DictVal(a) -> (match a with
                           ...
                           |Vuoto -> Val((id, rt_eval v r), Vuoto)
                           |Val(x,xs) -> (let rec reflect x1 = match x1 with
                                            |(a,b) -> (*Se la chiave corrisponde a quella da inserire -> exception*)
                                                if a = id then failwith("Chiave già presente")
                                                else let r = DictVal(xs) in 
                                                    Val((a, b), insert r)
                                          in reflect x))
          |_ -> failwith("type error")
...
let d0 = eval dec0 env1;;		      (*Dizionario con 2 stringhe*)

let dec1 = Dict(Elem(("Eta1", Eint(22)),
                     Elem(("Eta2", Eint(24)),
                          Empty)));;

let d1 = eval dec1 env1;;		      (*Dizionario con 2 oggetti interi*)
...
let zero = eval (Dict(Elem(("Primo elemento", Eint(15)),
...
(*Creazione dizionario con tipi errati
  let test0 = Dict(Elem(("Test0", Minus(Ebool(true))),
  Empty));;
  eval test0 env1;;
